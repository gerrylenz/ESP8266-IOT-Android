...
import com.espressif.esp8266_iot.EsptouchResult;
import com.espressif.esp8266_iot.IEsptouchResult;
import com.espressif.esp8266_iot.protocol.EsptouchGenerator;
import com.espressif.esp8266_iot.udp.UDPSocketClient;
import com.espressif.esp8266_iot.udp.UDPSocketServer;
import com.espressif.esp8266_iot.util.ByteUtil;
import com.espressif.esp8266_iot.util.EspNetUtil;
...
public class __EsptouchTask implements __IEsptouchTask {
	...
	private void __checkTaskValid() {
		// !!!NOTE: the esp8266_iot task could be executed only once
		if (this.mIsExecuted) {
	...
	public List<IEsptouchResult> executeForResults(int expectTaskResultCount)
		...
		if (Looper.myLooper() == Looper.getMainLooper()) {
			throw new RuntimeException(
					"Don't call the esp8266_iot Task at EsptouchDemoActivity(UI) thread directly.");
		}
		InetAddress localInetAddress = EspNetUtil.getLocalInetAddress(mContext);
		if (__IEsptouchTask.DEBUG) {
			Log.i(TAG, "localInetAddress: " + localInetAddress);
		}
		// generator the esp8266_iot byte[][] to be transformed, which will cost
		...
		// listen the esp8266_iot result asyn
